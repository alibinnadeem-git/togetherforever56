...
export default function RolesClient({ canCreate, canUpdate }:{ canCreate:boolean; canUpdate:boolean }) {
  ...
  return <div className="grid gap-6 lg:grid-cols-[.9fr_1.4fr]">
    ...
    <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-5 sm:p-6">
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm">Name<input value={name} onChange={e=>setName(e.target.value)} disabled={selected&&!canUpdate} className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 outline-none"/></label><label className="text-sm">Key<input value={key} onChange={e=>setKey(e.target.value)} disabled={!!selected} placeholder="chapter_event_manager" className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 outline-none disabled:opacity-50"/></label></div>
      <label className="mt-4 block text-sm">Responsibilities / description<textarea value={description} onChange={e=>setDescription(e.target.value)} disabled={selected&&!canUpdate} rows={3} className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 outline-none"/></label>
